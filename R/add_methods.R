...
#'
#' Methods for the [`AzureGraph::ms_graph`], [`AzureGraph::az_user`], [`AzureGraph::az_group`] and [`AzureGraph::ms_object`] classes.
#'
#' @rdname add_methods
#' @name add_methods
#' @section Usage:
...
#'
#' ## R6 method for class 'ms_object'
#' make_basic_list(op, filter, n, ...)
#' ```
#' @section Arguments:
...
#' - `op` A string, the REST operation.
#' - `...` Arguments passed to lower-level functions, and ultimately to `AzureGraph::call_graph_endpoint`.
#' @section Details:
...
#'
#' `make_basic_list` creates a list of Graph objects. A basic utility called by various Microsoft365R class methods. It is exported to work around issues in how R6 handles classes that extend across package boundaries. It should not be called by the user. If `n` is NULL, returns an iterator object, of class `AzureGraph::ms_graph_pager`. Otherwise, returns a list of individual Graph objects.
#'
...
#'
#' For `make_basic_list` if `n` is NULL, an iterator object of class `AzureGraph::ms_graph_pager`. Otherwise, a list of individual Graph objects.
#' @seealso
#' [`ms_site`], [`ms_drive`], [`ms_plan`], [`ms_team`], [`ms_chat`], [`az_user`], [`az_group`], [`az_object`]
#' @examples
#' \dontrun{
#'
